...
# Modified by: James Pino <james.ch.pino@gmail.com>
# License: MIT
...
item1 = itemgetter(1)
_wp_dir = os.path.dirname(wordcloud.__file__)
FONT_PATH = os.environ.get("FONT_PATH",
                           os.path.join(_wp_dir, "DroidSansMono.ttf"))

STOPWORDS = set([x.strip() for x in
                 open(os.path.join(_wp_dir, 'stopwords')).read().split('\n')])
...
class WordCloud(object):
    ...
    def __init__(self, font_path=None, width=400, height=200, margin=2,
                 ...
                 max_font_size=None, font_step=1, mode="RGB", min_count=0,
...
def unigrams_and_bigrams(words, normalize_plurals=True):
    ...
    for bigram_string, count in counts_bigrams.items():
        ...
        # print(count, counts[word1], counts[word2], n_words)
        if count == counts[word1] or counts[word1] == count and count != 1:
            keepers.append(bigram_string)
        # print(word1, word2, count, counts[word1],counts[word2], score(count, counts[word1], counts[word2], n_words))
        if score(count, counts[word1], counts[word2], n_words) > 25:
    ...
    for word in words:
        ...
        # print(word, counts_unigrams[word])
